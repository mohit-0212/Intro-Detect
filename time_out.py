...
def out(vid_path):
	vid_frame = cv2.VideoCapture(vid_path)
	cover = cv2.cvtColor(cv2.imread('./static/bbt.jpg'), cv2.COLOR_BGR2GRAY)
	...
	mins = str(int(time[maximum_score]/60))
	secs = str(int(time[maximum_score]%60))
	# print(intro_end)
	return mins, secs
